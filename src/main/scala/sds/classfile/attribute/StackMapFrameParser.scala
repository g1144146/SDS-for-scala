...
object StackMapFrameParser {
    private def getBefore(parsed: Link[(Int, Int), Map[String, Buffer[String]]], before: (Int, Int)): Buffer[String] =
        parsed.getOrElse(before, Map()).getOrElse("local", Buffer()).clone()
        ...
        frames.foreach((frame: Frame) => {
            ...
            val key: Int = frame match {
                ...
                case app: AppendFrame =>
                    app.locals.foreach(local += parseVerify(_, pool, opcodes))
                    map.put("stack", Buffer())
                    map.put("local", local)
                    app.offset
                case full: FullFrame =>
                    val ffStack: Buffer[String] =
                        full.stacks.map(parseVerify(_, pool, opcodes)).toBuffer.asInstanceOf[Buffer[String]]
                    val ffLocal: Buffer[String] =
                        full.locals.map(parseVerify(_, pool, opcodes)).toBuffer.asInstanceOf[Buffer[String]]
                    map.put("stack", ffStack)
                    map.put("local", ffLocal)
                    full.offset
                case chop: ChopFrame =>
                    val delArg: Int = 251 - chop.tag
                    (0 until delArg).foreach((_ :Int) => local.remove(local.size - 1))
                    map.put("stack", Buffer())
                    map.put("local", local)
                    chop.offset
                case s: SameFrame =>
