	...
	def extract(target: ConstantInfo, pool: Array[ConstantInfo]): String = {
		target.getTag() match {
			...
			case CLASS   => {
				...
				// need to removeJavaLangPrefix 
				extract(pool(c.getIndex() - 1), pool).replace("/", ".")
			}
			...
			case NAME_AND_TYPE => {
				...
				// need to parse last
				extract(pool(name.getName() - 1), pool) + "|" + extract(pool(name.getDesc() - 1), pool)
			}
			case HANDLE         => extract(pool(target.asInstanceOf[Handle].getIndex() - 1), pool)
			// need to parse
			case TYPE           => extract(pool(target.asInstanceOf[Type].getDesc() - 1), pool)
